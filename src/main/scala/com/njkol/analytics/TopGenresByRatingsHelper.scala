 ...
 */
object TopGenresByRatingsHelper extends Logging {
   ...
   * @param numOfEp The number of episodes if applicable
   * @return SQL for Join
   */
  def animeToUserRatingSQL(typ: String, numOfEp: String): String = {
    ...
    fromBuilder.append("anime_data a inner join user_ratings u on a.anime_id = u.anime_id ")

    fromBuilder.append("where ")
    fromBuilder.append(s"a.type = '${typ}' ")
    fromBuilder.append(s"and a.episodes > ${numOfEp} ")

    finalQueryBuiler.append(selectBuilder.toString()).append(fromBuilder.toString()).append(whereBuilder.toString())
   ...
   */
  def weightedRating = (rating: String, viewership: String, minUsers: Long, meanRating: Double) => {
